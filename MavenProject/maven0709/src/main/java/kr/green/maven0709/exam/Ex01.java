...
public class Ex01 {
	...
	public void test() {
		...
		assertEquals(solution.solution(new String[] {"mislav", "stanko", "mislav", "ana"}, new String[] {"stanko", "ana", "mislav"}),"mislav");
	}
	static class Solution {
	    public String solution(String[] participant, String[] completion) {
	         ...
	         for ( i=0; i < completion.length; i++){
	            if (!participant[i].equals(completion[i])){ // 이름이 같지 않으면 같지 않음 사람의 이름
	                return participant[i];
	            }
	        }
	        return participant[i]; // 모두 반복했는데 이름이 남았다면 남은 사람의 이름
	    }
	}
}

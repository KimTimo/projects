...
<body>
	...
	${mvo }
	 --%>
	<%
		// 1. 서비스를 호출하여 아이디 찾기를 수행한다.
		String userid = MemberService.getInstance().findUserid(mvo);
	
		request.setAttribute("userid", userid);
		// 2. 삭제 성공하면 강제 로그아웃을 시키고 아래의 버튼을 눌러 홈으로 이동 하도록 하자!! 
	%>
	<c:if test='${userid!="" }'>
		<h1>${mvo.username }님의 아이디는 ${userid }입니다.</h1>
		<h1>${userid }로 로그인하시기 바랍니다.</h1>
		<button  class="btn btn-outline-success btn-sm" onclick="location.href='login.jsp'">로그인하기</button>
	</c:if>	
	<c:if test='${userid=="" }'>
		<h1>${mvo.username }님의 정보는 존재하지 않습니다.</h1>
		<h1>회원가입을 하시려면 회원가입 단추를 클릭하여 가입을 진행해 주세요 </h1>
		<button  class="btn btn-outline-success btn-sm" onclick="location.href='findUserId.jsp'">다시하기</button>

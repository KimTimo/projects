...
<body>
	...
	${mvo }
	<%
		// 1. 이메일 변경시 이메일 인증을 다시 해야 한다.
		// 2. 서비스를 호출하여 수정을 수행한다.
		boolean isUpdate = MemberService.getInstance().update(mvo);
		// 3. 수정이 성공하면 세션의 정보를 수정해준다.
		if(isUpdate){
			session.setAttribute("mvo", mvo);
		}
	
	%>
	<h1>${mvo.username }님의 정보가 정상적으로 수정되었습니다.</h1>
	<h1>이메일을 변경하셨다면 ${mvo.email }에서 인증을 하셔야 회원 정보 수정이 완료됩니다.</h1>

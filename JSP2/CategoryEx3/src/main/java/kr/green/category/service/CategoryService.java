	...
	public void deleteItem(CategoryVO vo) {
		...
		try{
			...
			if(vo.getLev()==0) { // lev가 0이면 대항목
				...
				// 소항목 삭제
				// 1) 일단 자신을 지운다.
				categoryDAO.deleteByIdx(sqlSession, vo.getIdx());
				// 2) ref가 같으면서 seq가 나보다 큰것 모두 얻기
				List<CategoryVO> list = categoryDAO.selectNextSeq(sqlSession, vo);
				if(list!=null && list.size()>0) {
					// 3) 반복문을 돌리면서 삭제를 해나가다 
					for(CategoryVO cvo : list) {
						// 나의 레벨과 같거나 적은 레벨이 나온다면 종료한다.
						if(cvo.getLev()<=vo.getLev()) break;
						categoryDAO.deleteByIdx(sqlSession, cvo.getIdx());
					}
				}
			}
			// -----------------------------------------------------------------------

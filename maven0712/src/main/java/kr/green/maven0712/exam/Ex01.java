package kr.green.maven0712.exam;
...
public class Ex01 {
	...
	public void test() {
		Solution solution = new Solution();
		assertEquals(solution.solution("one4seveneight"), 1478);
		assertEquals(solution.solution("23four5six7"), 234567);
		assertEquals(solution.solution("2three45sixseven"), 234567);
		assertEquals(solution.solution("123"), 123);
	}
	static class Solution {
	    public int solution(String s) {
	        String[] word = "zero,one,two,three,four,five,six,seven,eight,nine".split(",");
	        Map<String, Integer> map = new HashMap<>();
	        for(int i=0;i<word.length;i++) map.put(word[i], i);
	        //System.out.println(map);
	        for(String t : word) {
	        	if(s.contains(t))
	        		s = s.replace(t, map.get(t)+"");
	        }

...
<%
	...
	List<DeptVO> list = DeptService.getInstance().selectDept(dept_id, job_id);
	request.setAttribute("list", list);
	List<String> deptList = DeptService.getInstance().selectDeptID();
	request.setAttribute("deptList", deptList);
	List<String> jobList = DeptService.getInstance().selectJobID();
...
<script type="text/javascript">
	$(function(){
		$("#deptID").change(function(){
			...
			var jobid = $("#jobID").val();
			location.href="?id=" + deptid + "&job=" + jobid;
		});
			...
			<td colspan="6" class="info">
				...
				<select id="jobID">
					...
					<c:forEach var="job" items="${jobList }">
						<option value="${job }" ${param.job==job ? "selected='selected'" : "" }>${job }</option>
					</c:forEach>
				</select>
			</td>
		</tr>

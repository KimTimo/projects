...
public class Ex01 {
	...
	public void testSolusion() {
		Solution solution = new Solution();
		assertArrayEquals(solution.solution(new int[] { 44, 1, 0, 0, 31, 25 }, new int[] { 31, 10, 45, 1, 6, 19 }),
				new int[] { 3, 5 });
		assertArrayEquals(solution.solution(new int[] { 0, 0, 0, 0, 0, 0 }, new int[] { 38, 19, 20, 40, 15, 25 }),
				new int[] { 1, 6 });
		assertArrayEquals(solution.solution(new int[] { 45, 4, 35, 20, 3, 9 }, new int[] { 20, 9, 3, 45, 4, 35 }),
				new int[] { 1, 1 });
	}
	...
	static class Solution {
		...
		public int lottoScore(int[] lottos, int[] win_nums) {
			// int배열을 Integer List로 만들기1
			// List<Integer> list = new ArrayList<Integer>();
			// for(int i : lottos) list.add(i);

			// int배열을 Integer List로 만들기2
		...
		public int getRank(int count) {
			/*
			 * 순위 당첨 내용 1 6개 번호가 모두 일치, 2 5개 번호가 일치, 3 4개 번호가 일치, 4 3개 번호가 일치, 5 2개 번호가 일치,
			 * 6(낙첨) 그 외
			 */
			int rank = 7 - count;
			if (count <= 1)

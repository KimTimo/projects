...
public class Ex01 {
	...
	public void test() {
		Solution solution = new Solution();
		assertEquals(solution.solution(new int[] {3,1,2,3}), 2);
		assertEquals(solution.solution(new int[] {3,3,3,2,2,4}), 3);
		assertEquals(solution.solution(new int[] {3,3,3,2,2,2}), 2);
	}
	static class Solution {
	    public int solution(int[] nums) {
	        int length = nums.length/2; // 최대 마리수
	        // Set<Integer> set = new HashSet<>(IntStream.of(nums).boxed().collect(Collectors.toList())); // 중복을 제거
	        Set<Integer> set = new HashSet<>();
	        for(int n : nums) set.add(n);
	        // System.out.println(set);
	        // 중복제거한 마리수가 최대보다 적다면 중복제거한 마리수를 가지고
	        // 크다면 최대 마리수를 가진다.
	        return length>set.size() ? set.size():length; 
	    }
	}
}

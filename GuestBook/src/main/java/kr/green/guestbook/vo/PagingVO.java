...
public class PagingVO<T> {
	...
	public String getPageList() {
		...
		if(startPage>1) {
			...
			message += "<a class='page-link' href='javascript:SendPost(\"?\",{\"p\":\""+ (startPage-1) +"\",\"s\":\""+pageSize+"\",\"b\":\""+blockSize+"\"})' aria-label='Previous'>";
		...
		for(int i=startPage;i<=endPage;i++) {
			if(i==currentPage) {
				...
				message += "<li class='page-item'><a class='page-link' href='javascript:SendPost(\"?\",{\"p\":\""+ (i) +"\",\"s\":\""+pageSize+"\",\"b\":\""+blockSize+"\"})'>" + i + "</a></li>";
			}
		}
		// <%-- 다음 : 마지막 페이지가 전체페이지보다 적다면 다음이 있다 --%>
		if(endPage<totalPage) {
			message += "<li class='page-item'>";
			message += "<a class='page-link' href='javascript:SendPost(\"?\",{\"p\":\""+ (endPage+1) +"\",\"s\":\""+pageSize+"\",\"b\":\""+blockSize+"\"})' aria-label='Next'>";

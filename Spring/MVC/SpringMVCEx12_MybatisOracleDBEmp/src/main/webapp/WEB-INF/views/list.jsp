<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@  taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
<script src="https://code.jquery.com/jquery-3.5.1.min.js" ></script>
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js"></script>
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css">
<script	src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js"></script>
<script type="text/javascript">
	function editForm(id, name, role){
		//alert(id + "\n" + name + "\n" + role);
		$("#eid").val(id);
		$("#ename").val(name);
		$("#erole").val(role);
		$("#ename").focus();
	}
	function deleteForm(id, name, role){
		//alert(id + "\n" + name + "\n" + role);
		$("#did").val(id);
		$("#dname").val(name);
		$("#drole").val(role);
		$("#deleteBtn").focus();
	}
</script>

<style type="text/css">
	div {border: 1px solid gray;margin-bottom: 5px; padding: 10px;}
</style>
</head>
<body>
	<c:if test="${empty list }">
		<div style="color: red;text-align: center;">등록된 내용이 없습니다.</div>
	</c:if>
	<c:if test="${not empty list }">
		<c:forEach var="vo" items="${list }">
			<div> 
			${vo.id }. ${vo.name } (${vo.role})
			<input type="button" value="수정"  onclick="editForm(${vo.id},'${vo.name }','${vo.role}')" />
			<input type="button" value="삭제"  onclick="deleteForm(${vo.id},'${vo.name }','${vo.role}')"  />
			</div>
		</c:forEach>
	</c:if>
	<form action="insertOk" method="post">
		<input type="text" name="name" id="name" required="required" placeholder="이름"/>
		<input type="text" name="role" id="role" required="required" placeholder="직업"/>
		<input type="submit"  value="저장"/>
	</form>
	<form action="updateOk" method="post">
		<input type="text" name="id" id="eid" readonly="readonly" size="3"/>
		<input type="text" name="name" id="ename" required="required" placeholder="이름"/>
		<input type="text" name="role" id="erole" required="required" placeholder="직업"/>
		<input type="submit"  value="수정"/>
	</form>
	<form action="deleteOk" method="post">
		<input type="text" name="id" id="did" readonly="readonly" size="3"/>
		<input type="text" name="name" id="dname" readonly="readonly"/>
		<input type="text" name="role" id="drole" readonly="readonly"/>
		<input type="submit"  value="삭제" id="deleteBtn"/>
	</form>
	<a href="${pageContext.request.contextPath }">홈으로</a>
</body>
</html>
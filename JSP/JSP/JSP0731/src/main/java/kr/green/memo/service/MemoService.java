...
public class MemoService {
	...
	public PagingVO<MemoVO> selectList(int currentPage, int pageSize, int blockSize) {
		...
		try {
			...
			// -----------------------------------------------
			// 여기
			int totalCount = dao.getCount(conn); // 전체 개수
			pagingVO = new PagingVO<MemoVO>(currentPage, pageSize, blockSize, totalCount); // 페이지 계산완료
			// 1페이지 분량만 가져와서
			List<MemoVO> list = dao.selectList(conn, pagingVO.getStartNo(), pagingVO.getPageSize());
		...
		try {
			...
			if (memoVO != null) { // 객체가 전달되었느냐?
				if (memoVO.getName() != null && memoVO.getName().trim().length() > 0) { // 이름이 존재하냐?
					memoVO.setName(memoVO.getName().trim()); // 혹시라도 있을수 있는 앞뒤의 공백을 제거
					if (memoVO.getPassword() != null && memoVO.getPassword().trim().length() > 0) { // 암호가 존재하냐?
						memoVO.setPassword(memoVO.getPassword().trim()); // 혹시라도 있을수 있는 앞뒤의 공백을 제거
						if (memoVO.getContent() != null && memoVO.getContent().trim().length() > 0) { // 내용이 존재하냐?
							memoVO.setContent(memoVO.getContent().trim()); // 혹시라도 있을수 있는 앞뒤의 공백을 제거
							// 여기까지 들어왔다는 것은 모든 내용이 있는 데이터이다. 그럼 저장만 하면 된다.
	...
	public int delete(MemoVO memoVO) {
		...
		try {
			...
			if (memoVO != null) { // 객체가 전달되었느냐?
				if (memoVO.getPassword() != null && memoVO.getPassword().trim().length() > 0) { // 암호가 존재하냐?
					memoVO.setPassword(memoVO.getPassword().trim()); // 혹시라도 있을수 있는 앞뒤의 공백을 제거
					// 여기까지 들어왔다는 것은 모든 내용이 있는 데이터이다. 그럼 삭제를 하면 된다.
					// 비밀번호가 같을때만 삭제을 한다!!!!
					// 1. DB에 저장된 비밀번호를 가져온다,
					MemoVO dbVO = dao.selectByIdx(conn, memoVO.getIdx()); // 현재 글번호의 글을 가져온다.
					// 2. 현재 빌번호와 같으면 수정을 하고 다르면 수정을 하지 않는다.

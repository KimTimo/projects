...
<body>
	...
	<%
		PagingVO<BoardVO> pagingVO = BoardService.getInstance().selectList(cv.getCurrentPage(), cv.getPageSize(), cv.getBlockSize());
		...
		<c:if test="${not empty pv.list }">
			<c:forEach var="vo" items="${pv.list }" varStatus="vs">
				<tr align="center">
					<td>${vo.idx }</td>
					<td align="left" >
						<a href="#" onclick='SendPost("view.jsp",{"p":${pv.currentPage },"s":${pv.pageSize },"b":${pv.blockSize },"idx":${vo.idx },"m":"view","h":"true"},"post")'><c:out value="${vo.subject }"></c:out></a> 
						<%-- 여기에 댓글의 개수를 출력한다.  --%>
		...
		<tr>
			<td class="info" colspan="5">
				<button type="button" class="btn btn-outline-success btn-sm" 
			        onclick='SendPost("update.jsp",{"p":${pv.currentPage },"s":${pv.pageSize },"b":${pv.blockSize },"m":"insert"},"post")'>글쓰기</button>
			</td>
		</tr>
	</table>
</body>
